      ...
      try {
        ...
        if (doc.exists) {
          ...
          switch (router.name) {
            ...
            case 'acciones-email':
              commit('mostrarExito', getters.saludo + ', tu contraseña fue cambiada exitosamente.', { root: true })
              router.push({ name: 'home' })
              break;
          }
        ...
        commit('mostrarError', 'Ocurrio un error mostrando tu infomación', { root: true })
      }
    },
    cerrarSesion({ commit }) {
      auth.signOut()
